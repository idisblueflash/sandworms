...
exports.pointById= function (db, req, res) {
  ...
  var query = " SELECT      coder.name, SUM(complexity) AS score, SUM(complexity)/2 AS avg FROM     sandworms.things_of_coders,     sandworms.coder WHERE     coder.id = things_of_coders.coder_id         AND catalog = 'task'         AND coder_id = ? GROUP BY coder_id LIMIT 0, 100"
	db.query(
		query,
    [coder_id, weeks],
		function (err, rows) {
...
exports.utility= function (db, req, res) {
  var query ="SELECT      coder.name, COUNT(sandworms.things_of_coders.id) AS utility FROM     sandworms.things_of_coders,     sandworms.coder WHERE     coder.id = things_of_coders.coder_id         AND catalog = 'task' GROUP BY coder_id LIMIT 0, 100";
	db.query(
		query,
		function (err, rows) {
			if (err) throw err;
