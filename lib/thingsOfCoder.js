...
exports.offensiveById= function (db, req, res) {
  var coder_id = req.params.id;
  var query = " SELECT      coder.name, SUM(complexity) AS score FROM     sandworms.things_of_coders,     sandworms.coder WHERE     coder.id = things_of_coders.coder_id         AND catalog = 'task'         AND coder_id = ? GROUP BY coder_id LIMIT 0, 100"
	db.query(
		query,
    [coder_id],
		function (err, rows) {
